...
class SimCSE(object):
    ...
    def similarity(self, queries: Union[str, List[str]], 
        ...
        query_vecs = self.encode(queries, device=device, return_numpy=True, max_length=200) # suppose N queries
        
        if not isinstance(keys, ndarray):
            key_vecs = self.encode(keys, device=device, return_numpy=True ,max_length=200) # suppose M keys
        else:
            key_vecs = keys
